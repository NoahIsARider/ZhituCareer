...
| 4 | `python3 -m pytest`（数据洞察 + 分析历史升级后） | **128 passed** | 25.91s |

第 4 轮在新增「数据洞察统计」与「分析历史记录」功能后执行，新增 8 个用例全部通过，旧用例无回归。

## 测试文件分布

| 测试文件 | 用例数 | 覆盖范围 |
|----------|--------|----------|
| tests/test_app.py | 54 | Flask 端到端：认证/注册/限流/管理分页搜索/单条增删改/职业分析/职位与课程匹配/错误处理/JSON 安全/统计洞察/分析历史 |
...
| `GET /api/stats/overview` | 返回职位/课程统计、薪资分桶、城市分布、技能排行、行业趋势 |
| `GET /api/history` → 分析 → 回查 → `DELETE` | 历史保存、列表、删除全链路正常 |
